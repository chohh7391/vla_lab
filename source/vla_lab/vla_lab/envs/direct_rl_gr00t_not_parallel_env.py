...
from .utils.gr00t_service import ExternalRobotInferenceClient
import time


class DirectRLGr00tNotParallelEnv(DirectRLEnv):

    def __init__(self, cfg: DirectRLEnvCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg=cfg, render_mode=render_mode, **kwargs)

        self.gr00t_chunk_size = 16
        self.gr00t_policy = ExternalRobotInferenceClient(host="localhost", port=5555)
        print("Initialize Gr00t Client Node")

        self.gr00t_actions = None
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        if self.episode_length_buf[0] % self.gr00t_chunk_size == 0:
            ...
            gr00t_observations = self._get_gr00t_observations()
            gr00t_actions = self.gr00t_policy.get_action(gr00t_observations)
            ...
            self.gr00t_actions = groot_actions_delta_pose # [64, 16, 6]

            # forge env contain predicition of sucess, so we expand gr00t actions to match the num envs
            if self.action_space.shape[-1] != self.gr00t_actions.shape[-1]:
                self.gr00t_actions = torch.cat(
                    (self.gr00t_actions, torch.zeros((self.num_envs, self.gr00t_chunk_size, 1), device=self.device)), dim=-1
                )
        ...
        self._pre_physics_step(action + self.gr00t_actions[:, self.episode_length_buf[0] % self.gr00t_chunk_size, :])
